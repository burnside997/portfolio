...
if(get_post('teacher_scheduled_submit')){
  if(regist_month_scheduled($db, $ym, $user['user_id'], $get_days)){
    set_message('出勤可能日を送信しました。');
  } else{
    set_error('送信に失敗しました。');
  }
}

if(get_post('teacher_scheduled_delete_submit')){
  if(delete_month_scheduled($db, $user['user_id'], $ym)){
    set_message('出勤可能日を削除しました。');
  } else{
    set_error('削除に失敗しました。');
  }
}
...
$month_teacher_scheduled = get_month_scheduled($db, $sql, $user['user_id'], $ym . '-01');

$days_checked_checkbox = get_checked_checkbox($month_teacher_scheduled, $option['period']);

$admin_check_teacher_calendar = get_admin_check_teacher_calendar($db, $user['user_id']);

include_once VIEW_PATH . 'teacher_view.php';