<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modal1">出勤可能日入力</button>
<div class="modal fade" id="modal1" tabindex="-1" role="dialog" aria-labelledby="label1" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">

      <div class="modal-header">
        <h5 class="modal-title" id="label1">出勤可能日</h5>
      ...
      <form method="post">
        ...
        <div class="modal-footer">
          <?php if($month_teacher_scheduled !== []): ?>
            <p>当月の予定は送信済みです。修正される場合は削除ボタンをクリックしてから再度入力してください。</p>
          <?php endif; ?>
          <button type="button" class="btn btn-secondary" data-dismiss="modal">閉じる</button>
          <?php if($month_teacher_scheduled === []): ?>
            <button type="submit" class="btn btn-primary" name="teacher_scheduled_submit" value="送信">送信</button>
          <?php else: ?>
            <button type="submit" class="btn btn-primary" name="teacher_scheduled_delete_submit" value="削除">削除</button>
          <?php endif; ?>
        </div>
